...
save_root = pathlib.Path('/mnt/hd0/project_large_files/bca_grading/suqh/ssl_swin_pure_ssl')
save_root.mkdir(exist_ok=True)
data_trans = dict(
    test = A.Compose([
                        A.PadIfNeeded(min_height=patch_size*scale, min_width=patch_size*scale, border_mode=cv2.BORDER_REFLECT),
    ...
    train = A.Compose([
                        A.PadIfNeeded(min_height=patch_size*scale, min_width=patch_size*scale, border_mode=cv2.BORDER_REFLECT),
                        A.RandomCrop(patch_size*scale, patch_size*scale),
                        A.Resize(patch_size, patch_size),
...
def train_one_epoch(model, optimizer, data_loader, device, epoch):
    ...
    ssl_loss_function = torch.nn.MSELoss()
    ...
    for step, data in enumerate(data_loader):
        ...
        ssl_features = ssl_model(images).detach()
        ...
        ssl_loss = ssl_loss_function(features, ssl_features)
        # loss = 0.5*loss + 0.5*ssl_loss
        loss = ssl_loss
...
@torch.no_grad()
def evaluate(model, ssl_model, data_loader, device, epoch):
    loss_function = torch.nn.CrossEntropyLoss()
    ssl_loss_function = torch.nn.MSELoss()
    ...
    for step, data in enumerate(data_loader):
        ...
        features = model.forward_features(images).detach()
        ssl_features = ssl_model(images)
        pred = model.forward(images)
        pred_classes = torch.max(pred, dim=1)[1]
        accu_num += torch.eq(pred_classes, labels).sum()
