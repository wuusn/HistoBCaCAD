...
def plot_roc(roc,
    ...
    # Set some defaults.
    label = label if label != None else "Feature"
    zorder = kwargs.pop("zorder", 1)
    legend_out = kwargs.pop("legend_out", False)
    legend_label_inv = kwargs.pop("legend_label_inv", True)
    if legend_label_inv:
        auc_disp, auc_val = "1-AUC" if roc.inv else "AUC", roc.auc_mean
    else:
        auc_disp, auc_val = "AUC", roc.auc
    auc_lower = roc.auc95_ci[0][0]
    auc_upper = roc.auc95_ci[0][1]
    # label = "%s%s = %.2f (%.2f - %.2f)" % (label, auc_disp, auc_val, auc_lower, auc_upper)
    label = "%s%s = %.4f (%.4f - %.4f)" % (label, auc_disp, auc_val, auc_lower, auc_upper)
    # label = "%s%s = %.4f" % (label, auc_disp, auc_val)
...
def plot_mean_roc(rocs, ori_roc, auto_flip=True, show_all=False, ax=None, **kwargs):
    """
    ...
    show_ci:    Show confidence interval, default True
    show_ti:    Show tolerance interval, default False
    ...
    show_ci = kwargs.pop("show_ci", False)
    ...
    plot_roc(roc=ret_mean, ori_roc=ori_roc, label=label, show_opt=show_opt,
             color=color, ax=ax, zorder=zorder + 3, **kwargs)
...
def plot_roc_bootstrap(X, y, pos_label,
    ...
    # 1) Collect the data.
    rocs, ori_roc = compute_roc_bootstrap(X=X, y=y,
                                 pos_label=pos_label,
                                 objective=objective,
                                 auto_flip=auto_flip,
    ...
    ret_mean = plot_mean_roc(rocs=rocs,ori_roc=ori_roc, auto_flip=auto_flip, show_all=show_boots, ax=ax, **kwargs)
