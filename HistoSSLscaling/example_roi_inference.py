...
This script follows the two-stage notebook flow used in:
- `extract_roi_features.ipynb`
- `mil_roi_model_on_the_fly_lora.ipynb`

Pipeline:
1) Extract ROI tile features with iBOT ViT (+ optional LoRA checkpoint),
2) Run ROI-level ABMIL classification on extracted tile features.

Defaults use bundled assets:
...
def build_abmil_model(device: torch.device) -> nn.Module:
    from rl_benchmarks.models.slide_models.abmil import ABMIL

    return ABMIL(768, [3, 3], d_model_attention=128, temperature=1.0, mlp_hidden=[128, 64]).to(device)
...
def main() -> None:
    ...
    mil_model = build_abmil_model(device)
    mil_model.eval()
