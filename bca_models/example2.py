...
# save_root = pathlib.Path('/mnt/hd0/project_large_files/bca_grading/suqh/histosslscaling_finetune/2023_10_25_16:21:16_AUC_0.73/center_crop_features_10x_224')
...
for phase in phases:
    ...
    # _labels = [1 if label==0 else 0 for label in _labels] # grade 2,3 vs grade 1
    # _labels = [1 if label==2 else 0 for label in _labels] # grade 1,2 vs grade 3
    ...
    for i in range(len(feature_paths)):
        if _labels[i] == 0:
            new_labels.append(0)
            new_features.append(np.load(feature_paths[i]))
            new_names.append(feature_paths[i].stem)
        elif _labels[i] == 2:
            new_labels.append(1)
            new_features.append(np.load(feature_paths[i]))
            new_names.append(feature_paths[i].stem)
    labels[phase] = new_labels
    features[phase] = new_features
    names[phase] = new_names
...
df.to_csv(save_root / 'train_val_feature_1vs3.csv', index=None)

dfs['test'].to_csv(save_root / 'test_feature_1vs3.csv', index=None)
