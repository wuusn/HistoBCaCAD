...
from .encoders import vit_base, swin_base
...
class iBOTViT(Extractor):  # pylint: disable=abstract-method
    ...
    def forward(self, images: torch.Tensor) -> torch.Tensor:
            ...
            o = self.head(features)

            return o

class iBOTSwin(Extractor):  # pylint: disable=abstract-method
    ...
    def __init__(
        ...
        assert (
            architecture == "swin_base"
        ), "Weights are released for `swin_base` architecture only."
        self.feature_extractor = swin_base(
            window_size=7, return_all_tokens=True, masked_im_modeling=True,
        )
        self._weights_path = weights_path
        if self._weights_path is None:
            self._weights_path = MODEL_WEIGHTS["iBOTViTBasePANCAN"]
