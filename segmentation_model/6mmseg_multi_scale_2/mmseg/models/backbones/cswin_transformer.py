...
class LePEAttention(nn.Module):
    def __init__(self, dim, resolution, idx, split_size=12, dim_out=None, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
...
class CSWinBlock(nn.Module):
    ...
    def __init__(self, dim, patches_resolution, num_heads,
                 split_size=12, mlp_ratio=4., qkv_bias=False, qk_scale=None,
...
class CSWin(nn.Module):
    ...
    """
    def __init__(self, img_size=384, patch_size=4, in_chans=3, embed_dim=64, depth=[1,2,21,1], split_size = 12,
                 num_heads=[1,2,4,8], mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm, use_chk=False, pretrained=False):
        super().__init__()
        ...
        self.stage1 = nn.ModuleList([
            CSWinBlock(
                dim=curr_dim, num_heads=heads[0], patches_resolution=384//4, mlp_ratio=mlp_ratio,
        ...
        self.stage2 = nn.ModuleList(
            [CSWinBlock(
                dim=curr_dim, num_heads=heads[1], patches_resolution=384//8, mlp_ratio=mlp_ratio,
        ...
        temp_stage3.extend(
            [CSWinBlock(
                dim=curr_dim, num_heads=heads[2], patches_resolution=384//16, mlp_ratio=mlp_ratio,
        ...
        self.stage4 = nn.ModuleList(
            [CSWinBlock(
                dim=curr_dim, num_heads=heads[3], patches_resolution=384//32, mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias, qk_scale=qk_scale, split_size=split_size[-1],
                drop=drop_rate, attn_drop=attn_drop_rate,
        ...
        # self.stage4 = nn.ModuleList(
            # [CSWinBlock(
                ...
                # drop_path=dpr[np.sum(depth[:-1])+i], norm_layer=norm_layer, last_stage=True)
            # for i in range(depth[-1])])
    #def init_weights(self, pretrained='checkpoints/cswin_large_384.pth'):
    def init_weights(self):
        ...
        if isinstance(pretrained, str):
            ...
            self.load_state_dict(state_dict, False)
             # for key in ckpt['state_dict_ema']:
                # # print(key)
